      SUBROUTINE Xabclib_GMRES (N, NNZ, IRP, ICOL, VAL, B, X,
...
*
      ...
      DOUBLE PRECISION  ETIME3,PERR,EMA,MIN_ETIME
*
      IDBG = IATPARAM(50)
*
...
*
*  INITIALIZE beta & v_1 FOR ALNOLDI'S PROCEDURE
*
         CALL Xabclib_GMRES_INIT(N,NNZ,IRP,ICOL,VAL,X,B,WORK(IP_Z),
     $           WORK(IP_V),BETA,BETA0,WORK(IP_IDIAGP),
     $           KIND_PRECOND,MOPE,PRECOND,IATPARAM,RATPARAM,ITRCNT,
     $           RERR,UINF,LUINF,INFO)
...
*
         IF (IDBG .EQ. 1) THEN
            WRITE(6,6000) ITRCNT-1,RERR
         END IF
         IF (RERR.LE.STOP_TOL .OR. ITRCNT-1.EQ.MAX_ITER) GO TO 1000
         ETIME3= ETIME2
         ETIME2= OMP_GET_WTIME()
...
*
         CALL Xabclib_GMRES_GSPOL(N,NNZ,IRP,ICOL,VAL,WORK(IP_IDIAGP),
     $           KIND_PRECOND,MOPE,PRECOND,IATPARAM,RATPARAM,
     $           WORK(IP_H),M,M0,WORK(IP_V),WORK(IP_WJ),WORK(IP_Z),
     $           UINF,LUINF,INFO)
         FLOP=FLOP+4.0D0*NNZ*M+4.0D0*N*M+2.0D0*M*M*N
         IATPARAM(31)=IATPARAM(31)+M0
         IATPARAM(32)=IATPARAM(32)+M0
         IF (INFO .NE. 0) THEN
            IF (IDBG .EQ. 1) WRITE(6,*) ' # MGS,  INFO=',INFO
            GO TO 9999
         END IF
*                                          _
...
*--------right precon.
*==============
         CALL Xabclib_GMRES_MIN(N,WORK(IP_H),M,M0,WORK(IP_G),WORK(IP_V),
     $           BETA,X,
     $           NNZ,IRP,ICOL,VAL,WORK(IP_IDIAGP),
     $           KIND_PRECOND,MOPE,PRECOND,IATPARAM,RATPARAM,WORK(IP_Z),
     $           INFO)
         FLOP=FLOP+2.0D0*M*N+2.0D0*NNZ
         IF (INFO .NE. 0) THEN
            IF (IDBG .EQ. 1) WRITE(6,*) ' # MIN,  INFO=',INFO
            GO TO 9999
         END IF
      END DO
*
...
*
      ETIME2= OMP_GET_WTIME()
      IF (RERR.GT.STOP_TOL .AND. ITRCNT-1.EQ.MAX_ITER) THEN
         INFO=500
      END IF
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
*
      IATPARAM(23)=ITRCNT-1
      IATPARAM(29)=MSIZE
      IF (MSIZE_AT .EQ. 1) THEN
         IF (IRT .EQ. 1) THEN
            IATPARAM(29)=MOLD
      ...
      RATPARAM(28)=BETA0
      RATPARAM(29)=RERR
      FLOP=FLOP+(ITRCNT-1)*(24.0D0*N+8.0D0*NNZ)
...
*-----------
      IF (IDBG .EQ. 1) THEN
         WRITE(6,*) '=================================================='
         IF (INFO .EQ. 0) THEN
            WRITE(6,*) ' ## GMRES PROCESS SUCCESSFULLY ENDED.'
         ELSE
            WRITE(6,*) ' ## GMRES PROCESS FAILED.'
         END IF
         WRITE(6,6001) ITRCNT-1,RERR,ETIME2-ETIME1
         WRITE(6,*) '=================================================='
      END IF
*
